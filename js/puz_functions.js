...
function puzdata_to_pdf(xw, options) {
    ...
    var margin = options.margin;
    var side_margin_left = options.side_margin_left;
    var side_margin_right = options.side_margin_right;
    ...
    var title_xpos = side_margin_left + options.header_indent;
    ...
    var max_width = DOC_WIDTH - (side_margin_left + side_margin_right);
    ...
    var author_xpos = DOC_WIDTH - side_margin_right;
    ...
    if (options.right_header) {
        ...
        max_width = DOC_WIDTH - ((side_margin_left + side_margin_right) + doc.getTextWidth(title[0]) + title_right_margin);
        ...
        if (author_align == 'left') {
            author_xpos = title_width + side_margin_left + title_right_margin;
        }
    ...
    var subheader_xpos = side_margin_left + options.subheader_indent;
    ...
    if (options.subheader && subheader_text) {
        ...
        max_width = DOC_WIDTH - (side_margin_left + side_margin_right);
        ...
        if (subheader_align == 'left') {
            ...
            subheader_xpos = DOC_WIDTH - side_margin_right;
    ...
    // size of columns
    var col_width = (DOC_WIDTH - (side_margin_left + side_margin_right) - (options.num_columns - 1) * options.column_padding) / options.num_columns;

    // The grid is under all but the first few columns
    var grid_width = DOC_WIDTH - (side_margin_left + side_margin_right) - options.num_full_columns * (col_width + options.column_padding);

    // If only two columns, grid size is limited
    if (options.columns == "new" || options.num_columns == 2) {
        grid_width = DOC_WIDTH - (side_margin_left + side_margin_right);
    }
    ...
    var grid_xpos = DOC_WIDTH - side_margin_right - grid_width;

    if (options.grid_placement == "left") {
        grid_xpos = side_margin_left;
    }
    ...
    while (finding_font && !manual_spacing) {
        ...
        var num_xpos = side_margin_left + num_margin;
        ...
        for (var k = 0; k < clue_arrays.length; k++) {
            ...
            for (var i = 0; i < clues.length; i++) {
                ...
                if ((line_ypos + ((lines.length - 1) * (clue_pt + line_padding))) > max_line_ypos) {
                    ...
                    my_column += 1;
                    num_xpos = side_margin_left + num_margin + my_column * (col_width + options.column_padding);
    ...
    if (options.copyright) {
        ...
        if (options.grid_placement == 'left') {
            copyright_xpos = (side_margin_left + grid_width);
        } else {
